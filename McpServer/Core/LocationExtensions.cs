...
public static class LocationExtensions {
    public static SymbolLocation GetSymbolLocation(this Location location, Workspace workspace) {
        if (!location.IsInSource)
            throw new InvalidOperationException("Location is not in the source tree");

        var lineSpan = Path.GetExtension(location.SourceTree.FilePath).Equals(".cake", StringComparison.OrdinalIgnoreCase) ||
            location.SourceTree.FilePath.EndsWith("razor__virtual.cs", StringComparison.OrdinalIgnoreCase) ||
            location.SourceTree.FilePath.EndsWith("cshtml__virtual.cs", StringComparison.OrdinalIgnoreCase)
        ...
        var fileName = Path.IsPathRooted(lineSpan.Path) || generatedInfo != null
            ...
            : Path.GetFullPath(Path.Combine(Path.GetDirectoryName(location.SourceTree.FilePath) ?? string.Empty, lineSpan.Path));
